...
mcmc_routine = function( par, par_index, A, W, B, Y, x, z, steps, burnin, ind, trialNum, Dn_omega, simulation){
    ...
    mpi = list( c(par_index$vec_init),
                ...
                c(par_index$vec_A),
                c(par_index$vec_R, par_index$vec_R_bleed))
    ...
    for(ttt in 1:steps){
        ...
        for(j in 1:n_group) {
            ...
            if(j <= 3) {
                ...
                ind_j_new = list()
                ind_j_new[[1]] = par_index$vec_R
                ind_j_new[[2]] = par_index$vec_R_bleed
                for(jjj in 1:2) {
                    if(jjj == 1) {
                        S_update = curr_psi_nu[[1]]
                        nu_update = curr_psi_nu[[2]]
                    } else {
                        S_update = curr_psi_nu[[3]]
                        nu_update = curr_psi_nu[[4]]
                    }
                    proposal[ind_j_new[[jjj]]] = c(rinvwishart(nu = nu_update, S = S_update))
                    
                    curr_R = matrix(par[ind_j_new[[jjj]]], nrow = 4)
                    prop_R = matrix(proposal[ind_j_new[[jjj]]], nrow = 4)
                    
                    log_prop = dinvwishart(Sigma = prop_R, nu = nu_update, S = S_update, log = T)
                    log_prop_prev = dinvwishart(Sigma = curr_R, nu = nu_update, S = S_update, log = T)
                    ...
                    if( log_target + log_prop_prev - log_target_prev - log_prop > log(runif(1,0,1)) ){
                        log_target_prev = log_target
                        par[ind_j_new[[jjj]]] = proposal[ind_j_new[[jjj]]]
                        if(jjj == 1) {
                            accept[j] = accept[j] +1
                        } else {
                            accept[j+1] = accept[j+1] +1
                        }
                    }
                }
            }
