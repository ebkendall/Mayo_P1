...
# EID_ordering = sample(EIDs[-which(EIDs %in% c(194350, 234375, 259825, 288775, 747775))], 
#                       size = 495, replace = F)
# for(e in 2:length(EID_group)) {
#     EID_group[[e]] = EID_ordering[(5*(e-1)+1):(5*(e-1)+5)]
# }
...
for(o in 1:4) {
    for(p in 2:5) {
        ...
        save(sampling_out, 
             file = paste0('Model_out/sampling_out_',samp_type,'_',t_pt_length, 
                           '_rep', e_num, '.rda'))
    }
}
